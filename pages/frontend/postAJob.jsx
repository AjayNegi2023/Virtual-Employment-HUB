...
export default function PostAJob() {
    ...
    const [formData, setFormData] = useState({ user: user?._id, title: "", salary: 0, email: "", company: "", description: "", job_category: "", job_type: "", job_experience: "", job_vacancy: 0, job_deadline: "", location: "", company_logo_url: "", job_url: "" });
    const [error, setError] = useState({ user: "", title: "", salary: "", email: "", company: "", description: "", job_category: "", job_type: "", job_experience: "", job_vacancy: "", job_deadline: "", location: "", company_logo_url: "", job_url: "" });
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.job_url) {
            setError({ ...error, job_url: "job_url Field is required" })
            return;
        }

        if (!formData.company_logo_url) {
            setError({ ...error, company_logo_url: "company_logo_url Field is required" })
            return;
        }
        ...
        <>
            ...
            <div className='w-full  py-20 flex items-center  justify-center flex-col'>
                ...
                <form onSubmit={handleSubmit} className="sm:w-1/2 w-full px-4 mx-4  h-full" >
                    ...
                    <div className='w-full mb-4 flex flex-col items-start justify-center'>
                        <label htmlFor="jobUrl" className='mb-1 text-base font-semibold'>Job URL :</label>
                        <input onChange={(e) => setFormData({ ...formData, job_url: e.target.value })} type="text" id='jobUrl' className='w-full py-2 px-3 mb-2 border border-indigo-600 rounded' placeholder='Enter URL of the job' />
                        {error.job_url && <p className="text-sm text-red-500">{error.job_url}</p>}
                    </div>
                    <div className='w-full mb-4 flex flex-col items-start justify-center'>
                        <label htmlFor="companyLogoUrl" className='mb-1 text-base font-semibold'>Company Logo URL :</label>
                        <input onChange={(e) => setFormData({ ...formData, company_logo_url: e.target.value })} type="text" id='companyLogoUrl' className='w-full py-2 px-3 mb-2 border border-indigo-600 rounded' placeholder='Enter URL of the company logo' />
                        {error.company_logo_url && <p className="text-sm text-red-500">{error.company_logo_url}</p>}
                    </div>
                    <div className='w-full mb-4 flex flex-col items-start justify-center'>
                        <label htmlFor="location" className='mb-1 text-base font-semibold'>Location :</label>
                        <input onChange={(e) => setFormData({ ...formData, location: e.target.value })} type="text" id='location' className='w-full py-2 px-3 mb-2 border border-indigo-600 rounded' placeholder='Enter location of the job' />
