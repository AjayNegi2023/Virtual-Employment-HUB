...
export default function NavBar() {
    ...
    const handlePostJobClick = () => {
        if (user?.role === undefined || user?.role === null) {
            ...
            setOpenJobs(state => !state);
        }
    };

    return (
        <>
            <div className={`w-full ${scrolled ? "bg-teal-600/70" : "bg-teal-600"} px-6 h-20 bg-indigo-600 text-white flex items-center justify-between fixed top-0 left-0 z-50`}>
                <div className='px-2 h-full flex items-center justify-center'>
                    <p className='uppercase font-semibold tracking-widest text-lg'>UUPoint</p>
                </div>
                <div className='px-2 h-full hidden items-center justify-center lg:flex'>
                    ...
                    <Link href={'/courses/Home/CourseHome'} className="px-3 mx-4 text-base font-medium transition-all duration-700 hover:translate-y-2 uppercase" >Courses</Link>
                ...
                {isOpen && (
                    <div ref={ref} className='flex w-full py-2 animate-fade-in-down  bg-teal-400 transition-all fade duration-1000 absolute top-20 left-0  items-center justify-center flex-col '>
                        <div className='px-2 h-full flex items-center justify-center flex-col py-2 '>
                            ...
                            <button onClick={handlePostJobClick} className="px-3  m-4 text-base font-medium transition-all duration-700 hover:translate-y-2 uppercase flex items-center justify-center" >Jobs {openJobs ? <AiFillCaretUp /> : <AiFillCaretDown />} </button>
                            {openJobs && (
                                <>
